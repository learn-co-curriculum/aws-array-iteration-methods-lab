...
describe('index.js', function () {
  describe('findMatching(driversArray, driverName)', function () {
    it('returns all drivers that match the passed in name', function () {
      const drivers = ['Edith', 'Sammy', 'Marte', 'Mina', 'Camron', 'Mina']

      expect(findMatching(drivers, 'Mina')).to.eql(['Mina', 'Mina']);
    ...
    it('returns an empty array if there is no match', function () {
      ...
      expect(findMatching(drivers, 'Lou')).to.eql([]);
    });
  });
  ...
  describe('addCurrentAge(people, currentYear)', function() {
    it("adds each person's currentAge as a property on their object", function() {
      const peopleArr = [
        { name: "Meredith", birthYear: 1985 },
        { name: "Hasung", birthYear: 1994 },
        { name: "Julio", birthYear: 1963 }
      ]
      addCurrentAge(peopleArr, 2022) 
      expect(peopleArr[1].currentAge).to.equal(28)
    })
  })
});
